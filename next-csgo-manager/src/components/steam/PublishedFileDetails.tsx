...
export default function PublishedFileDetails({
  workshopId,
}: {
  workshopId: number;
}) {
  ...
  const { data, error, isLoading } = useSWR(
    `/api/steam/IPublishedFileService/GetDetails/${workshopId}`,
    fetcher
  );
      ...
      <div>
        <div className="card max-w-xl bg-base-100 shadow-xl">
          ...
          <img
            ...
            alt="preview"
            className="rounded-t-box object-cover"
          />
          {/* <Image
            src={data.preview_url}
            className="rounded-t-box object-cover"
          ...
          ></Image> */}
          <div className="card-body">
            <div className="flex justify-items-start items-center justify-between">
              <h2 className="card-title">{data.title}</h2>
              <div className="flex gap-1">
                {data.tags?.map((tag) => (
                  <span key={tag.tag} className="badge badge-neutral">
                    {tag.display_name}
                  </span>
                ))}
              </div>
