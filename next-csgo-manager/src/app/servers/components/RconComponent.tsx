...
export default function RconComponent({ serverId }: { serverId: string }) {
  ...
  const handleCommand = async (rcon_command: string) => {
    if (!session || !session.user) {
      console.error("Can't handle input without user session");
  ...
  if (!session) {
    return (
      <div role="alert" className="alert alert-error">
        ...
        <span>Error! You have to be logged in to use RCON.</span>
      </div>
    );
  }
